...
class UnifiedWalletConnection {
  ...
  showUserFriendlyError(error, chain) {
    ...
    // Detect if user is already in a wallet browser
    ...
    if (error.includes('No compatible wallets')) {
      if (isMobile && !isInWalletBrowser) {
        // Mobile user not in wallet browser - provide specific guidance
        message += 'On mobile devices, you need to open this website inside your wallet app\'s browser. ' +
                   'Open your crypto wallet app, look for a "Browser" or "DApp Browser" section, ' +
                   'and navigate to this website from there.';
      } else {
        // Desktop or already in wallet browser - original message
