...
class WildWestWallet {
    ...
    try {
      ...
      if (availableWallets.length === 0) {
        if (isMobile) {
          ...
          // Check if user is in wallet browser
          const isInWalletBrowser = /MetaMask|Trust|Coinbase|Rainbow|WalletConnect/i.test(navigator.userAgent) || 
                                    window.ethereum;
          if (!isInWalletBrowser) {
            throw new Error('On mobile devices, you need to open this website inside your wallet app\'s browser. Open your crypto wallet app, look for a "Browser" or "DApp Browser" section, and navigate to this website from there.');
          }
          return false;
        } else {
    ...
    try {
      ...
      if (availableWallets.length === 0) {
        if (isMobile) {
          ...
          // Desktop user - standard wallet installation message
          throw new Error('No EVM wallet detected. Please install MetaMask, Coinbase Wallet, or another Web3 wallet.');
        }
      }
  ...
  showStatus(message, type = 'info') {
    ...
    // For error messages, add FAQ reference
    if (type === 'error') {
      statusDiv.innerHTML = `
        <div style="margin-bottom: 8px;">${message}</div>
        <div style="font-size: 0.9em; opacity: 0.9;">
          💡 Need help? Check our <strong>FAQ</strong> button below for troubleshooting tips!
        </div>
      `;
    } else {
