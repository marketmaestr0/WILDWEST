...
(function(){
  ...
  async function fetchDexPair(token){
    try { 
      ...
      // CRITICAL FIX: Filter pairs to exact token address match to prevent data mixup
      if (!j?.pairs) return null;
      
      const exactPairs = j.pairs.filter(pair => {
        const baseAddress = pair.baseToken?.address?.toLowerCase();
        const quoteAddress = pair.quoteToken?.address?.toLowerCase();
        const targetAddress = token.toLowerCase();
        return baseAddress === targetAddress || quoteAddress === targetAddress;
      });
      
      if (exactPairs.length === 0) return null;
      
      // Prefer Base chain, but ensure exact address match
      return exactPairs.find(p=> (p.chainId||'').toLowerCase()==='base') || exactPairs[0] || null;
    } catch { 
      return null; 
    }
  }
